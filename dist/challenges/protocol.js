...
class ChallengeProtocol {
     ...
     */
    processChallenge(store, decisionId, challenger, action, rationale, counterProposal, structuredCounterProposal) {
        ...
        if (decision.status === 'accepted' || decision.status === 'escalated' || decision.status === 'superseded') {
            throw new Error(`Decision ${decisionId} is already ${decision.status}, cannot process further challenges`);
        }
        if (action === 'accept') {
        ...
        if (structuredCounterProposal) {
            createdCP = this._createCounterProposal(decisionId, updated.challenge_rounds, challenger, structuredCounterProposal);
        }
     ...
     * @internal
     */
    _createCounterProposal(decisionId, round, proposedBy, params) {
     ...
     * @internal
     */
    _supersedePendingCounterProposals(decisionId, notes) {
     ...
     */
    resolveCounterProposal(counterProposalId, status, notes) {
     ...
     */
    getCounterProposals(decisionId) {
     ...
     */
    getCounterProposal(counterProposalId) {
        for (const [, cps] of this.counterProposals) {
            const cp = cps.find(c => c.id === counterProposalId);
            if (cp)
                return cp;
     ...
     */
    getAuditTrail(store, query) {
        const all = store.all();
        const entries = [];
        for (const d of all) {
            // Only include decisions that have been through the challenge process
            if (d.challenge_rounds === 0 && d.status === 'proposed')
                continue;
            // Apply filters
            if (query) {
                if (query.author && d.author !== query.author)
                    continue;
                if (query.project && d.project !== query.project)
                    continue;
                if (query.phase !== undefined && d.phase !== query.phase)
                    continue;
                if (query.escalated !== undefined && (d.status === 'escalated') !== query.escalated)
                    continue;
                if (query.minRounds !== undefined && d.challenge_rounds < query.minRounds)
                    continue;
                if (query.challenger) {
                ...
                if (query.after && d.timestamp < query.after)
                    continue;
                if (query.before && d.timestamp > query.before)
                    continue;
                if (query.hasCounterProposals !== undefined) {
                    const cps = this.counterProposals.get(d.id) ?? [];
                    if (query.hasCounterProposals && cps.length === 0)
                        continue;
                    if (!query.hasCounterProposals && cps.length > 0)
                        continue;
                }
        ...
        for (const entry of entries) {
            ...
            if (entry.counter_proposals.length > 0) {
                ...
                for (const cp of entry.counter_proposals) {
                    const statusIcon = cp.status === 'accepted' ? '✅' :
                        cp.status === 'rejected' ? '❌' :
