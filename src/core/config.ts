...
import type { BoardConfig, RoleConfig } from './types.js';
...
export class ConfigLoader {
  ...
  resolvePrompt(
    ...
    const fullPath = resolve(this.baseDir, promptPath);
  ...
  private buildTemplateVars(
    ...
    const gatekeeperRole = Object.entries(config.roles).find(
      ([_, r]) => r.responsibilities.includes('gate_verdicts') || r.responsibilities.includes('validation')
    );
    ...
    const strategist = Object.entries(config.roles).find(
      ([_, r]) => r.responsibilities.includes('planning')
    );
    const architect = Object.entries(config.roles).find(
      ([_, r]) => r.responsibilities.includes('architecture') || r.responsibilities.includes('design_review')
    );

    if (strategist) vars.strategist_role = strategist[1].title;
    if (architect) vars.architect_role = architect[1].title;
  ...
  private resolveAgentDirectory(
    ...
    const eachMatch = template.match(
      /\{\{#each agents\}\}([\s\S]*?)\{\{\/each\}\}/
    );
    if (!eachMatch) return template;
  ...
  private validate(config: BoardConfig): void {
    if (!config.name) throw new Error('Board config missing "name"');
      ...
      if (role.challenges) {
        for (const target of role.challenges) {
          if (!config.roles[target]) {
            throw new Error(
              `Role "${roleName}" challenges non-existent role "${target}"`
            );
          }
        }
      }
      ...
      for (const [gateName, gate] of Object.entries(config.gates)) {
        for (const required of gate.required) {
          if (!config.roles[required]) {
            throw new Error(
              `Gate "${gateName}" requires non-existent role "${required}"`
            );
          }
        }
      }
